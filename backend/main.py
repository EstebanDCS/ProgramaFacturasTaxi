...
# Función ultra-segura para escribir. Si choca con una celda combinada secundaria, busca la principal.
def escribir(ws, celda, valor):
    if valor is None: return
    try:
        ws[celda] = valor
    except AttributeError:
        # Si da el error 'MergedCell', buscamos la coordenada maestra
        try:
...
async def generar_documento(datos: DatosFactura):
    try:
        ...
        escribir(hoja_invoice, 'E15', datos.factura_numero) # Nº Factura
        escribir(hoja_invoice, 'C17', datos.barco)          # Barco principal

        total_importe = 0.0
        fechas_servicio_obj = []
        fila_actual_invoice = 21 # Empieza la descripción
        ticket_num = 1
        ...
        for t in datos.tickets:
            ...
            # --- DATOS DEL TICKET (Nuevas coordenadas exactas) ---
            escribir(ws_ticket, 'E9', t.contacto_metodo)
            escribir(ws_ticket, 'C12', t.fechas_solicitud) # Modificado a C12
            escribir(ws_ticket, 'B14', t.fecha_servicio)
            escribir(ws_ticket, 'C16', datos.barco)
            escribir(ws_ticket, 'B19', t.pasajeros)        # Confirmado B19
            escribir(ws_ticket, 'B46', t.comentarios)      # Confirmado B46
            ...
            if t.origen_tipo == 'hotel': escribir(ws_ticket, 'E28', t.origen_texto_extra) # Modificado a E28
            
            escribir(ws_ticket, 'B30', '☑' if t.origen_tipo == 'otros' else '☐')
            if t.origen_tipo == 'otros': escribir(ws_ticket, 'D30', t.origen_texto_extra) # Modificado a D30
            
            # --- DESTINO ---
            escribir(ws_ticket, 'B31', '☑' if t.destino_tipo == 'aeropuerto' else '☐')
            escribir(ws_ticket, 'B32', '☑' if t.destino_tipo == 'buque' else '☐')
            
            escribir(ws_ticket, 'B35', '☑' if t.destino_tipo == 'hotel' else '☐')
            if t.destino_tipo == 'hotel': escribir(ws_ticket, 'E35', t.destino_texto_extra) # Modificado a E35
            
            escribir(ws_ticket, 'B36', '☑' if t.destino_tipo == 'hospital' else '☐')
            if t.destino_tipo == 'hospital': escribir(ws_ticket, 'E36', t.destino_texto_extra) # Modificado a E36
            
            escribir(ws_ticket, 'B37', '☑' if t.destino_tipo == 'clinica' else '☐')
            if t.destino_tipo == 'clinica': escribir(ws_ticket, 'E37', t.destino_texto_extra) # Modificado a E37
            
            escribir(ws_ticket, 'B38', '☑' if t.destino_tipo == 'inmigracion' else '☐')
            
            escribir(ws_ticket, 'B40', '☑' if t.destino_tipo == 'otros' else '☐')
            if t.destino_tipo == 'otros': escribir(ws_ticket, 'D40', t.destino_texto_extra) # Modificado a D40
        ...
        if fechas_servicio_obj:
            escribir(hoja_invoice, 'F17', max(fechas_servicio_obj).strftime("%d/%m/%Y")) # Modificado a F17
